...
(function () {
  ...
  function measure() {
    ...
    if (M.deepOn) {
      ...
      /* נקודת האפס של deep. בכל המצבים היא נקשרת לרגע שבו
         הסקשן שמעליו סיים את שלו, ולא לראש הסקשן עצמו — אחרת
         נשאר מסך שלם של גלילה מתה ביניהם.
         דסקטופ: הרגע שהרכבת עוצרת.
         מובייל: הרגע שהבמה של הפרוצס משתחררת מהנעיצה, כלומר
         מתחילה להיגלל החוצה. הכותרת של deep היא fixed ולכן היא
         עולה כבר בזמן שהפרוצס עוד על המסך — נמדד שאין ביניהם
         חפיפה. עד שהפרוצס היה ערימה סטטית זה לא היה אפשרי,
         והקומנט שהיה כאן תיאר את המצב ההוא.
         ‏768–1199 נשאר ערימה סטטית, ושם עדיין ראש הסקשן. */
      M.deepTop = (M.fxOn && process)
        ? M.processTop + M.processTravel * M.railEnd
        : (M.procMOn && process)
          ? M.processTop + M.processTravel
          : deep.offsetTop;
